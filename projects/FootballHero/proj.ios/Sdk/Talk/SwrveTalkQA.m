...
// HTTP Babble Requests
- (NSArray *)TEST_getTalkQARequests;

// HTTP Babble Responses
...
- (void) addTestLog:(NSString*)type message:(NSDictionary*)message;
...
-(id)initWithJSON:(NSDictionary*)qaJson withAnalyticsSDK:(Swrve*)swrve
{
    self = [super init];
    self.swrve = swrve;
    ...
    self.isTesting = self.swrve && [self.swrve TEST_getTalkQALog];
    
    return self;
}

-(BOOL)canMakeSessionRequest {
    if (self.swrve && self.logging) {
...
{
    if ([self canMakeSessionRequest] || self.isTesting) {
        ...
        if (self.isTesting) {
            [self addTestLog:@"session" message:talkSessionJson];
        }
    }
}
...
{
    if ([self canMakeTriggerRequest] || self.isTesting) {
        ...
        if (self.isTesting) {
            [self addTestLog:@"trigger" message:triggerJson];
        }
    }
}
...
-(void)trigger:(NSString*)event withMessage:(SwrveMessage*)messageShown withReason:(NSDictionary*)campaignReasons withMessages:(NSDictionary*)campaignMessages
{
    if ([self canMakeTriggerRequest] || self.isTesting) {
        ...
        if (messageShown != NULL) {
            NSMutableDictionary* campaignInfo = [[NSMutableDictionary alloc] init];
            [campaignInfo setValue:[NSNumber numberWithInteger:messageShown.campaign.ID] forKey:@"id"];
...
-(void)updateDeviceInfo
{
    ...
    if (self.isTesting) {
        [self addTestLog:@"device_info" message:deviceJson];
    }
}
...
-(void)pushNotification:(NSDictionary*)notification
{
    if ([self canMakeTriggerRequest]) {
        ...
        if (self.isTesting) {
            [self addTestLog:@"push" message:note];
        }
    }
}
...
-(void)makeRequest:(NSString*)endpoint withJSON:(NSMutableDictionary*)json
{
    ...
    NSMutableArray * testQARequests = (NSMutableArray*)([self.swrve TEST_getTalkQARequests]);
    if (testQARequests) {
        NSString * testEntry = [NSString stringWithFormat:@"%@%@", [requestURL absoluteString], [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding]];
        [testQARequests addObject:testEntry];
    }
    ...
    [[self swrve] sendHttpPOSTRequest:requestURL
                    ...
                    {
                        if (error) {
                            NSLog(@"Talk QA Error: %@", error);
                        } else if (![response isKindOfClass:[NSHTTPURLResponse class]]) {
                            NSLog(@"Talk QA response was not a HTTP response: %@", response);
                        } else {
                            NSHTTPURLResponse* httpResponse = (NSHTTPURLResponse*)response;
                            long status = [httpResponse statusCode];
                            NSString* responseBody = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
                                           
                            NSLog(@"HTTP Send to QA Log %ld", status);
                                           
                            if (status != 200){
                                NSLog(@"HTTP Error %ld while doing Talk QA", status);
                                NSLog(@"  %@", responseBody);
                            }
                        }
                                       
                        // Store test responses
                        NSMutableArray * testQAResponses = (NSMutableArray*)([self.swrve TEST_getTalkQAResponses]);
                        if (testQAResponses) {
                            ...
                            NSString * testResponse = [NSString stringWithFormat:@"[%@][%@]", statusString, json];
                            [testQAResponses addObject:testResponse];
                        }
                    }];
}
...
- (void) addTestLog:(NSString *)type message:(NSDictionary *)message
{
    ...
    [entry setObject:message forKey:@"message"];
    [((NSMutableArray*)[self.swrve TEST_getTalkQALog]) addObject:entry];
}
