        ...
        if currYear > startYear then
            ...
            while currYear >= startYear do
                ...
                for i = startMth, tempEndMth do
                    local displayDate = Constants.stringFormatWithVariableOrder( Constants.String.event.ranking_dropdown_month, months["m"..tostring(i)], startYear )
...
function initLeaderboardContent( i, content, info )
    ...
    string.format( Constants.String.leaderboard[mSubType["descriptionKey"]], info[mSubType["dataColumnId"]], info["NumberOfCoupons"] )
    index:setText( i )
