...
function loadFrame()
    mWidget = GUIReader:shareReader():widgetFromJsonFile("scenes/TutorialYuuzooSignin.json")
    ...
    signin:setTitleText( Constants.String.button.yuuzoo_sign_in )
...
function signinEventHandler( sender, eventType )
    if eventType == TOUCH_EVENT_ENDED then
        ...
        EventManager:postEvent( Event.Do_YZ_Connect, { email, pass } )
    end
end
